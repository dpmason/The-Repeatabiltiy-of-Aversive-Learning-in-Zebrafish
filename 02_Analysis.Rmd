...
##Learning Exp2
```{r}
learn_model_e2 <- lmer(difference ~ scale(day) + sex + scale(fish_size) + exp + (1 | fishID), data = e2)
...
x <- rpt(difference ~ sex + (1 | fishID), grname = "fishID", data = x, datatype = "Gaussian", nboot = 100, npermut = 100, adjusted = FALSE) #estimate of enhanced agree repeatability, differing from adjusted since the fixed effects variance is included in the denominator
...
x <- rpt(difference ~ 1 + (1 | fishID), grname = "fishID", data = x, datatype = "Gaussian", nboot = 100, npermut = 100, adjusted = FALSE)
...
##Single Condiion Repeatabilties
...
x <- rpt(difference ~ set + sex + (1 | fishID), grname = "fishID", data = x, datatype = "Gaussian", nboot = 100, npermut = 0)
...
x <- rpt(difference ~ set + (1 | fishID), grname = "fishID", data = x, datatype = "Gaussian", nboot = 100, npermut = 0)
...
x <- rpt(BASELINE ~ 1 + (1 | fishID), grname = "fishID", data = x, datatype = "Gaussian", nboot = 100, npermut = 100)
...
rename_rm_col(rbind(Red, Green, Check, Orange), c("Red", "Green", "Grey", "Orange")) -> pref_outputs
```
...
rename_rm_col(rbind(red_pref_male_output, red_pref_female_output, green_pref_male_output, green_pref_female_output, check_pref_male_output, check_pref_female_output, orange_pref_male_output, orange_pref_female_output), c("red_pref_male_output", "red_pref_female_output", "green_pref_male_output", "green_pref_female_output", "check_pref_male_output", "check_pref_female_output", "orange_pref_male_output", "orange_pref_female_output")) -> pref_outputs_sex #df pref rpt outputs sex separated
