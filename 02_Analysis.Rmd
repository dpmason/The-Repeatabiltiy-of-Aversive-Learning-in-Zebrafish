...
test <- lmer(difference ~ scale(day) + sex + scale(fish_size) + learning + (1 | exp) + (1 | fishID), data = e2)
summary(test)

test1 <- lmer(difference ~ scale(day) + scale(fish_size) + learning + (1 | exp) + (1 | fishID), data = e2)
summary(test1)
...
```
#Learning Within Conditions combined
```{r}
#within conditions
learn_model_within_combined <- lmer(difference ~ scale(day) + sex + scale(fish_size) + learning + (1|exp) + (1 | fishID), data = allgbbg)
summary(learn_model_within_combined)

test <- lmer(difference ~ scale(day) + sex + scale(fish_size) + (1 | fishID), data = allgbbg)
summary(test)

#across and within conditions
learn_model_both_combined <- lmer(difference ~  sex  - 1+ scale(day) + scale(fish_size) + learning_combined + (1 | exp) + (1 | fishID), data = allgbbg)
summary(learn_model_both_combined)

learn_model_both_combined <- lmer(difference ~  sex  + scale(day) + scale(fish_size) + learning_combined + (1 | exp) + (1 | fishID), data = allgbbg)
summary(learn_model_both_combined)
