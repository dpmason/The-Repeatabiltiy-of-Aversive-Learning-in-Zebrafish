...
```
Consider looking at diminishing learning response in probe period for supplementary
...
e1_data <- map(unlist(all_path), import_csv) %>% #uses import_csv function from the source script functions.R to modify the raw data into R suitable structure
  ...
  mutate(day = as.numeric(day)) #changes day varaible to numeric 
...
missing_probe <- read.csv("./data/missing_probe.csv") #loads missing probe data csv into a df
...
jointed2 <- read.csv("../hons/data/Experiment 2 Size and ID Data/e2_fishID_size.csv")
...
select_rename <- function(x) {
  x %>% select(tank.x, mark.x, updatedsex, fishID, fish_length, fish_width, test, filepath, day, BASELINE, PROBE, difference, fish_size, zone, exp, exp_group, learning, set) %>% rename(tank = tank.x, mark = mark.x, sex = updatedsex) -> e1_selected
}

select_rename(exp1) -> e1_selected

select_rename(ex1_reduced) -> e1_selected_reduced
...
saveRDS(e1, file = "e1.rds")

saveRDS(e2, file = "e2.rds")
```
