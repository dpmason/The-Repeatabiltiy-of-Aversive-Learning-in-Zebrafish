...
pacman::p_load(tidyverse, ggbeeswarm, rptR, lme4, ggpubr, ggforce, magrittr, wesanderson)
...
fig2B <- col_pref %>% 
  ...
  labs(x = "Colour", y = "Preference", title = "Colour Preference During the Baseline Period") +
...
sex_cols <- c("Female" = "#D55E00","Male" = "#009E73", "Mixed" = "#999999")

rpt_outputs_all %<>% mutate(Sex = case_when (grepl("_Male", conditions) ~ "Male",
                                             grepl("_Female", conditions) ~ "Female",
                                             grepl("Green_Blue", conditions) ~ "Mixed",
                                             grepl("Blue_Green", conditions) ~ "Mixed",
                     ...
                     mutate(CS = case_when (grepl("Blue/", conditions) ~ "Blue",
                                            grepl("_Blue", conditions) ~ "Blue",
                                            grepl("Green/", conditions) ~ "Green",
                                            grepl("_Green", conditions) ~ "Green",
...
saveRDS(rpt_outputs_all, file = "rpt_outputs_all.rds")
```
...
probe_draw <- function(dat, condition = "", title = "") {
  ...
  labs(x = "Minute  Increments in Probe Period", y = "Time Spent in CS+ During Probe (seconds per minute) ± SE", title = title) +
  ylim(0,45)
  
  print(gplot)
}

probe_draw(e1_sum, condition = "REDYEL", title = "Red/Yellow Condition")
probe_draw(e1_sum, condition = "YELRED", title = "Yellow/Red Condition")
probe_draw(e1_sum, condition = "GREBLU", title = "Green/Blue Condition")
probe_draw(e1_sum, condition = "BLUGRE", title = "Blue/Green Condition")
probe_draw(e1_sum, condition = "CHKGRY", title = "Check/Grey Condition")
probe_draw(e1_sum, condition = "GRYCHK", title = "Grey/Check Condition")
probe_draw(e1_sum, condition = "ORAMAG", title = "Orange/Magenta Condition")
probe_draw(e1_sum, condition = "MAGORA", title = "Magenta/Orange Condition")
```
...
probe_draw_all(e1_sum_all, title =  "All Conditions")
```
