...
```{r}
##basic model
learn_model_e1 <- lmer(difference ~ scale(day) + sex + scale(fish_size) + learning + (1 | exp) + (1 | fishID), data = across_cond) #z-scaling, if you scale fish size you have to scale day as well

summary(learn_model_e1) #experiment 1 93 levels
tab_model(learn_model_e1) #to print out model results for extraction

learn_model_combined <- lmer(difference ~ scale(day) + sex + scale(fish_size) + learning_combined + (1 | exp) + (1 | fishID), data = allgbbg)
summary(learn_model_combined)
...
learn_model_e2_old <- lmer(difference ~ scale(day) + sex + scale(fish_size) + learning + (1 | exp) + (1 | fishID), data = e2)
